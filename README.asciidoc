...
cd docker
podman-compose --profile tools up cert-generator
----
...
----
# Edit docker/.env with your Cloudflare credentials and domain configuration
docker-compose --profile tools run deflector
----
...
- Test instance available (`docker-transporter-test/`)
- Standard ports: 3128 (main), 3129 (test)
...
│   └── squid-start.sh
├── docker-transporter-test/     # Squid test instance
│   ├── Dockerfile
│   ├── squid-ubuntu-hybrid.conf.template
│   └── ubuntu-init.sh
